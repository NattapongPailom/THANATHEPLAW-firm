...
export const Navbar: React.FC<NavbarProps> = ({ onSelectService, onNavigate, activeContext }) => {
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-700 ${isScrolled ? 'bg-slate-950/98 shadow-2xl py-2 sm:py-3 backdrop-blur-2xl border-b border-[#c5a059]/30' : 'bg-transparent py-4 sm:py-8'}`}>
      <div className="max-w-[1536px] mx-auto px-4 lg:px-10 flex justify-between items-center relative">
        {/* Logo Section */}
        <div className="flex-shrink-0 cursor-pointer" onClick={() => window.location.href = '/'}>
          <div className="text-xl sm:text-2xl xl:text-3xl font-bold tracking-tighter text-white whitespace-nowrap">
            <span className="text-[#c5a059] font-serif-legal italic">T</span>HANATHEP<span className="font-light tracking-[0.15em] ml-1 opacity-60 text-[8px] xl:text-xs uppercase">LAW FIRM</span>
          </div>
        </div>
        ...
        <div className="hidden lg:flex items-center flex-nowrap">
          {/* Language Switcher */}
          <div className="flex items-center mr-4 xl:mr-8 pr-4 xl:pr-8 border-r border-white/10 gap-4 xl:gap-6 flex-shrink-0">
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed inset-0 bg-slate-950 z-[200] transition-transform duration-700 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 flex flex-col h-full overflow-y-auto">
          <div className="flex justify-between items-center mb-12">
            <div className="text-2xl font-bold text-white"><span className="text-[#c5a059]">T</span>HANATHEP</div>
          ...
          <div className="space-y-6 flex-grow">
            {links.map((link) => (
              <div key={link.name}>
                <a 
                  href={link.href} 
                  className={`block text-3xl font-bold transition-colors ${activeSection === link.id ? 'text-[#c5a059]' : 'text-white'}`} 
                  onClick={() => handleLinkClick(link.href)}
                >
                  {link.name}
                </a>
