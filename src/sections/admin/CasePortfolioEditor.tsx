...
export const CasePortfolioEditor: React.FC<CasePortfolioEditorProps> = ({ lead, onClose, onRefresh }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      finalTimeline.push({
        ...
        time: eventTime,
        title: eventTitle,
        description: `อัปเดตเอกสารใหม่: ${fileName}`,
        isCompleted: true
      });
  ...
  const removeFile = async (fileId: string) => {
    if (!confirm("ยืนยันการลบไฟล์นี้จากระบบ Cloud?")) return;
  ...
  const removeTimeline = async (eventId: string) => {
    if (!confirm("ยืนยันการลบไทม์ไลน์นี้?")) return;
          ...
          <div className="bg-slate-950/50 p-6 sm:p-10 border border-white/5 space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-6">
              <div>
                ...
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black mt-1">
                  บันทึกส่วนตัวของทนาย หรือข้อความที่ต้องการแจ้งลูกความเพิ่มเติม
                </p>
              </div>
            ...
            <div className="relative">
              <textarea 
                ...
                placeholder="เขียนบันทึกยุทธศาสตร์ที่นี่..."
                value={leadNotes}
          ...
          <form onSubmit={handleSubmit} className="space-y-10 bg-slate-950/30 p-6 sm:p-10 border border-white/5">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest">ชื่องาน / ความคืบหน้า</label>
                <input required className="w-full bg-slate-950 border border-white/10 p-4 text-white text-sm outline-none focus:border-[#c5a059]" placeholder="เช่น ยื่นคำฟ้องต่อศาลแพ่ง" value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2 relative">
                  <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest">วันที่</label>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-start">
              ...
              <div className="space-y-4">
                <label className="text-[10px] uppercase font-black text-slate-500 tracking-widest flex items-center gap-2">
                  <Upload size={12} className="text-[#c5a059]" /> อัปโหลดเอกสาร
                </label>
                <div
                ...
                >
                  <div className="flex flex-col items-center gap-3 pointer-events-none">
                    ...
                    <div className="text-center">
                      {pendingFile ? (
                        <>
                          <p className="text-sm font-black uppercase tracking-widest">✓ ไฟล์ที่เลือก</p>
                        ...
                        <>
                          <p className="text-sm font-black uppercase tracking-widest">ลากไฟล์ที่นี่</p>
                          <p className="text-[9px] text-slate-500 mt-2">หรือคลิกเพื่อเลือกจากคอมพิวเตอร์</p>
                        </>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-slate-950/50 border border-white/5 p-4 rounded-sm">
              <p className="text-[9px] text-slate-400 italic">
                💡 <strong>เคล็ดลับ:</strong> เลือกไฟล์จากคอมพิวเตอร์หรือลากไฟล์เข้ามา ระบบจะอัปโหลดไปยัง Cloud Storage และส่งลิงก์ให้ลูกความผ่านทางอีเมล
              </p>
            </div>
